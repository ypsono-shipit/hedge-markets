...
function receipt(torn=false){const live=lines.map((l,i)=>`<div class="receipt-row">${i+1}. ${esc(l.category)} <b>BUY ${esc(l.side.toUpperCase())}</b><br>${esc(l.question)}<small>${esc(l.basisRisk)}</small><small>${l.quote&&l.quote.ask!=null?'Ask: $'+Number(l.quote.ask).toFixed(3)+(l.quote.eligible?'':' · thin book'):'No live quote'}</small></div>`).join('');const watch=gaps.map((g,i)=>`<div class="receipt-row">${lines.length+i+1}. ${esc(g.category)} <b>NO LIVE MARKET</b><br>${esc(g.title||g.category)}<small>${esc(g.reason||'')}</small><small>${esc(g.hedge)}</small></div>`).join('');const off=elsewhere.map((e,i)=>`<div class="receipt-row">${lines.length+gaps.length+i+1}. ${esc(e.category)} <b>ELSEWHERE</b><br>${esc(e.title)}<small>${esc(e.venue)} · ${esc(e.product)}</small><small>${esc(e.hedge)}</small></div>`).join('');return `<div class="printer-wrap"><div class="printer"><div class="printer-brand">Hedge Markets</div><div class="slot"></div></div><div class="receipt ${torn?'torn':''}" tabindex="0" role="group" aria-label="${torn?'Torn ticket. Drag to move it on the page.':'Ticket. Drag down to tear it, or slide it left and right.'}" style="--rx:${receiptPos.x}px;--ry:${receiptPos.y}px"><h3>HEDGE MARKETS</h3><div class="receipt-head"><b>${esc(company.name)}</b><br>${sourceStamp()}<br>${lines.length} LIVE · ${gaps.length} WATCH · ${elsewhere.length} ELSEWHERE<br>${company.url?esc(company.url):'No website used'}</div>${live}${watch}${off}<div class="receipt-warning">Not a broker or insurance.<br>Live rows are Polymarket contracts ($1 or $0).<br>Watchpoints have no live contract here.<br>Elsewhere rows are other venues — not traded here.<br>You can lose your stake.<br>Payouts may not offset business losses.</div></div>${torn?'':'<p class="receipt-hint">Grab the ticket — pull down to tear, slide left or right</p>'}</div>`}
...
function bindReceipt(){
  ...
  if(!paper)return;
  const torn=()=>paper.classList.contains('torn');
  ...
  paper.onpointermove=e=>{
    ...
    receiptPos.x=ox+dx;
    receiptPos.y=torn()?oy+dy:Math.max(0,oy+dy);
    apply();
  };
  const end=e=>{
    ...
    if(!torn()){
      if(receiptPos.y>=88){
        if(!saveTicket()){receiptPos.y=0;apply();return}
        receiptPos.y=Math.min(receiptPos.y,160);
        render('claim');
      }else{
        receiptPos.y=0;paper.classList.add('snap');apply();
        setTimeout(()=>paper.classList.remove('snap'),280);
      }
    }
  };
  paper.onpointerup=end;paper.onpointercancel=end;
  paper.onkeydown=e=>{
    if(e.key==='ArrowLeft'){receiptPos.x-=24;apply();e.preventDefault()}
    else if(e.key==='ArrowRight'){receiptPos.x+=24;apply();e.preventDefault()}
    else if(!torn()&&(e.key==='ArrowDown'||e.key==='Enter'||e.key===' ')){
      e.preventDefault();
      if(!saveTicket())return;
      receiptPos.y=110;render('claim');
    }
  };
}
...
if(p==='ticket'||p==='claim'){ensureDesk();main.innerHTML=`<section class="view"><div class="ticket-layout"><div class="ticket-copy"><h2>${p==='ticket'?'Your risks,<br>on one ticket.':'Take your list<br>with you.'}</h2><p>${esc(company.name)} · ${lines.length} live · ${gaps.length} watch · ${elsewhere.length} elsewhere</p><div class="warning"><b>A shortlist, not a safety net.</b>Live rows are Polymarket contracts. Watchpoints have no live market. Elsewhere rows point to other venues (exchanges, brokers, suppliers) — not traded here. Grab the paper itself — pull down to tear, slide it left or right.</div>${p==='claim'?`<div class="claim-actions" style="margin-top:20px">${currentId?'<div class="success">✓ Ticket saved on this browser.</div><button class="primary" id="hm-download">Download ticket ↓</button>':''}<button class="secondary" data-go="matches">Review events</button></div>`:''}${exportControls()}</div>${receipt(Boolean(currentId))}</div>${tradeDesk()}</section>`}
